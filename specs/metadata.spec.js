...
describe('Given the mappings', () => {
  ...
  const mappings = require('../generated/mwsfeeds').generated;
  let indent = 0;
  ...
  function toProperties(type) {
    ...
    const props = {};
    console.log('%sProcessing %j', suffix, type);
    ...
    if (type.localName && type.localName != 'Product.ProductData') {
      type.propertyInfos.map((prop) => {
        //console.log('%sprocessing property %j', suffix, prop);
        ...
        if (type === 'object') {
          if (!prop.typeInfo) throw new Error(`Can not convert prop=${JSON.stringify(prop)}`);
        ...
        console.log('%sResult=%j', suffix, result);
        props[name] = result;
      });
    }
    indent--;
